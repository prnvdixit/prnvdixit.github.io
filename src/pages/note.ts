...
export class NotePage extends AbstractView {
        ...
        noteContent.innerHTML = `
                    <div class="note-header">
                        <div class="note-header-top">
                            <h1>${note.title}</h1>
                            <button class="copy-link-btn" onclick="copyNoteLink('${
                              note.id
                            }')">Copy Link</button>
                        </div>
                        <div class="note-meta">
  ...
  private setupTableOfContents(element: HTMLElement): void {
    ...
    setTimeout(() => {
        ...
        (entries) => {
          ...
          toc.querySelectorAll(".toc-link").forEach((link) => {
            ...
            if (href) {
              ...
              if (
                targetHeading &&
                (targetHeading.tagName === "H1" ||
                  targetHeading.tagName === "H2" ||
                  href === "HCB_comment_box")
              ) {
      ...
      // Observe h1, h2 headings and Comments section for highlighting
      headings.forEach(({ id }) => {
  ...
  private setupScrollListeners(element: HTMLElement): void {
    ...
    if (floatingBackLink && originalBackLink) {
      const originalBackLinkRect = originalBackLink.getBoundingClientRect();
      const originalTop = originalBackLinkRect.top + 300;
