...
export class NotePage extends AbstractView {
  ...
  private renderSongs(content: string): Promise<string> {
    const lines = content.split("\n").filter((line) => line.trim() !== "");
    const sections: { title: string; songs: { title: string; url: string }[] }[] = [];
    let currentSection: { title: string; songs: { title: string; url: string }[] } | null = null;
          ...
          return `
            <div class="song-card">
              <a href="${song.url}" target="_blank" class="song-link">
                ...
                ${song.title ? `<div class="song-title">${song.title}</div>` : ""}
              </a>
            </div>
          `;
        }
